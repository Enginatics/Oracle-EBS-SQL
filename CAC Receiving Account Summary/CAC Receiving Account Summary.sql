...
-- Description: Summary report of the receiving accounting distributions by item, purchase order, purchase order line, release and project number. For outside processing, it includes the WIP job, OSP item number and the OSP resource code. Includes category column for expense destinations, (Note: this report has not been tested with encumbrance entries.)
