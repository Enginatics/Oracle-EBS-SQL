...
round(mcce.adjustment_quantity,coalesce(to_number(fnd_profile.value('INV:QUANTITY_PRECISION')),6)) adjustment_quantity,
round(nvl(mcce.adjustment_quantity,0)*nvl(mcce.item_unit_cost,0),gl.std_precision) adjustment_amount,
round(mcce.count_quantity_current,coalesce(to_number(fnd_profile.value('INV:QUANTITY_PRECISION')),6)) count_quantity_current,
round(mcce.neg_adjustment_quantity,coalesce(to_number(fnd_profile.value('INV:QUANTITY_PRECISION')),6)) negative_adjustment_quantity,
