...
xxen_util.meaning(nvl(rsl.asn_line_flag,'N'),'YES_NO',0) asn_line_flag,
decode(rsl.asn_line_flag,'Y',nvl(rsh.invoice_num,'@@'),'@@') asn_invoice_num,
...
nvl(rt.po_distribution_id,rsl.po_distribution_id) po_distribution_id
from
po_headers_all pha,
(
select
...
nvl(
 (select
  ...
  rcv_shipment_headers rsh
  where
  rsl.po_line_location_id=pda.line_location_id and
  rsl.asn_line_flag='Y' and
  rsh.shipment_header_id=rsl.shipment_header_id and
  aia.invoice_id=aida.invoice_id and
  aia.invoice_num=rsh.invoice_num
 ),
 '@@') asn_invoice_num
from
...
x.asn_invoice_num=aila.asn_invoice_num(+) and
(x.po_distribution_id is null or aila.po_distribution_id is null or x.po_distribution_id = aila.po_distribution_id) and
