...
-- Description: Detail report to get the accounting entries with amounts for each item, organization, and subinventory. The report Includes ship from and ship to information for inter-org transfers, but does not include WIP job information. You can also choose NOT to include Subinventory information, which will reduce the size of the report to only include the Accounting Line Type for inventory and Inventory Valuation
