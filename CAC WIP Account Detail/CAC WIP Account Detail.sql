...
from    wip_entities we,
        ...
        (select mp.organization_code,
                ...
                decode(wt.transaction_type,
                        4, (select description                                                      -- Std cost update
                            from   cst_cost_updates ccu
                            where  ccu.cost_update_id        = wt.cost_update_id),
                        regexp_replace(wt.reference,'[^[:alnum:]'' '']', null)
...
-- Revision for version 1.20
-- and     br_basis.resource_id (+)         = acct_dist.basis_resource_id
