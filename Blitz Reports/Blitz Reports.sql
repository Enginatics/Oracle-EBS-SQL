...
&modification
xrv.version,
(select max(xrh.creation_date) from xxen_reports_h xrh where xrv.report_id=xrh.report_id) sql_last_update_date,
xxen_util.user_name((select distinct max(xrh.created_by) keep (dense_rank last order by xrh.creation_date) over () from xxen_reports_h xrh where xrv.report_id=xrh.report_id)) sql_last_updated_by,
xrv0.report_name copied_from,
(select max(xrh.creation_date) from xxen_reports_h xrh where xrv.copied_from_guid=xrh.guid) copied_from_sql_last_upd_date,
xxen_util.user_name((select distinct max(xrh.created_by) keep (dense_rank last order by xrh.creation_date) over () from xxen_reports_h xrh where xrv.copied_from_guid=xrh.guid)) copied_from_sql_last_updatd_by,
xrv.db_package,
xrv.email,
xrv.output_format,
xrv.row_limit,
