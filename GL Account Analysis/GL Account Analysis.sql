...
(select pha.segment1 from po_headers_all pha where coalesce(aia.quick_po_header_id,rt.po_header_id,wt.po_header_id,decode(mmt.transaction_source_type_id,1,mmt.transaction_source_id,null))=pha.po_header_id) purchase_order,
--AR
case
...
ap_checks_all aca,
(select distinct aipa.check_id, max(aipa.invoice_id) keep (dense_rank last order by aipa.invoice_payment_id) over (partition by aipa.check_id) invoice_id from ap_invoice_payments_all aipa) aipa,
...
case when xte.application_id=200 then decode(xte.entity_code,'AP_INVOICES',xte.source_id_int_1,'AP_PAYMENTS',aipa.invoice_id) end=aia.invoice_id(+) and
