...
-- |
-- |  Parameters:
-- |  p_wip_var_threshold  -- Maximum WIP variance or current job balance that is allowed for jobs you wish to close
-- |  p_wip_var_percent    -- Maximum WIP variance percentage that is allowed for jobs you wish to close.  
-- |                         Based on WIP Job Balance / WIP Costs In.
-- |  p_include_scrap      -- Include scrapped assemblies in completion and component  material requirements
...
-- |  1.2     21 MAR 2017 Douglas Volz  Changed from attribute15 to attribute13, was already in use by a Yield Report.
...
-- |  1.5     25 May 2017 Douglas Volz  Added business code, product family, product line and package code item categories
...
-- |                                    and added the category set 1 description.  And now use multi-language tables or views.
...
-- +=============================================================================+*/
-- Excel Examle Output: https://www.enginatics.com/example/cac-wip-jobs-with-complete-status-but-not-ready-for-close/
-- Library Link: https://www.enginatics.com/reports/cac-wip-jobs-with-complete-status-but-not-ready-for-close/
...
having (
 ...
 -- ===========================================
   (abs(sum(nvl(wpb.pl_material_in,0)-
     nvl(wpb.tl_material_out,0)-
     nvl(wpb.pl_material_out,0)-
     nvl(wpb.tl_material_var,0)-
     nvl(wpb.pl_material_var,0) +
 ...
 -- ===========================================
 -- Check for transactions stuck in interfaces
 -- ===========================================
 or
         wdj.wip_entity_id in wip_interfaces.wip_entity_id
 -- ===========================================
 -- Check for WIP Completion Status
 -- ===========================================
 or
 -- Revision for version 1.10, check completion status
 -- Check for WIP Completion Status
 (wdj.start_quantity - (wdj.quantity_completed + decode(:p_include_scrap,'N',0, wdj.quantity_scrapped))) > 0
 -- ===========================================
 -- Check for WIP with no applied OSP
 -- ===========================================
 or
  -- Revision for version 1.1
  -- Check for WIP with no applied OSP
  wdj.wip_entity_id in
  (select wip_entity_id
   ...
   and    nvl(wor.applied_resource_units,0) = 0)
 -- ===========================================
 -- Check for WIP with material qtys not issued
 -- ===========================================
 or
  -- Revision for version 1.10
  -- Check for WIP with material quantities not issued
  wdj.wip_entity_id in
  (select wip_entity_id
   from   wip_requirement_operations wro,
   mtl_system_items_b msi,
