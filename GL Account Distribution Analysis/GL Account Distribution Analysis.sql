...
/*                                                                       */
/*************************************************************************/
-- Report Name: GL Account Distribution Analysis
...
-- Excel Examle Output: https://www.enginatics.com/example/gl-account-distribution-analysis/
-- Library Link: https://www.enginatics.com/reports/gl-account-distribution-analysis/
...
case
when xdl.application_id=200 and xdl.source_distribution_type='AP_PMT_DIST' then xdl.source_distribution_id_num_1
when xdl.application_id=200 and xdl.applied_to_distribution_type='AP_PMT_DIST' then xdl.applied_to_dist_id_num_1
end=aphd.payment_hist_dist_id(+) and
...
coalesce(case
when xdl.application_id=201 and xdl.source_distribution_type='PO_DISTRIBUTIONS_ALL' then xdl.source_distribution_id_num_1
when xdl.applied_to_distribution_type='PO_DISTRIBUTIONS_ALL' then xdl.applied_to_dist_id_num_1
end,aida.po_distribution_id,aila.po_distribution_id,decode(rrsl.reference1,'PO',to_number(rrsl.reference3)),rt.po_distribution_id)=pda.po_distribution_id(+) and
...
case
when xdl.application_id=201 and xdl.source_distribution_type='PO_REQ_DISTRIBUTIONS_ALL' then xdl.source_distribution_id_num_1
when xdl.application_id=201 and xdl.applied_to_distribution_type='PO_REQ_DISTRIBUTIONS_ALL' then xdl.applied_to_dist_id_num_1
end=prda.distribution_id(+) and
coalesce(case when xte.application_id=222 then case when xte.entity_code in ('TRANSACTIONS','BILLS_RECEIVABLE') then xte.source_id_int_1 when xte.entity_code='ADJUSTMENTS' then aaa.customer_trx_id end end,rctlgda.customer_trx_id)=rcta.customer_trx_id(+) and
...
case
when xdl.application_id=222 and xdl.source_distribution_type='RA_CUST_TRX_LINE_GL_DIST_ALL' then xdl.source_distribution_id_num_1
when xdl.application_id=222 and xdl.applied_to_distribution_type='RA_CUST_TRX_LINE_GL_DIST_ALL' then xdl.applied_to_dist_id_num_1
end=rctlgda.cust_trx_line_gl_dist_id(+) and
