...
(
 select
  ...
  pod.quantity_billed,
  (select ppx.full_name from per_people_x ppx where poh.agent_id=ppx.person_id) buyer,
  (select ppx.full_name from per_people_x ppx where pod.deliver_to_person_id=ppx.person_id) deliver_to_person,
